...
** volá funkci DLError().
**/
	tDLElemPtr elemPtr =malloc(sizeof(struct tDLElem));
	if (elemPtr == NULL) {
	    DLError();
	}
	elemPtr->data=val;
	elemPtr->rptr=L->First;
	elemPtr->lptr=NULL;
	if (L->Last == NULL){
	    L->Last=elemPtr;
	}
	else L->First->lptr=elemPtr;
