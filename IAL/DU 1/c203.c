...
if (q != NULL)
{
    for (int i=0;i<MAX_QUEUE;i++)
        q->arr[i]='*'; //všechny hodnoty v poli q->arr nastavte na '*',
    q->b_index=0; //index prvního volného místa nastavte také na 0.
    q->f_index=0; //index na začátek fronty nastavte na 0,
}
else {  //V případě, že funkce dostane jako parametr q == NULL, volejte funkci
    queueError(QERR_INIT);
}
...
*/
    if (queueEmpty(q) != 0){ // vraci nenulovou hodnotu pokud je fronta prazdna
        queueError(QERR_GET);
    }
    else {
        queueFront(q,c);
...
*/
    if (queueFull(q) != 0) // v pripade ze je fronta plna vraci nenulovou hodnotu
    {
        queueError(QERR_UP);
    }
    else
    {
        q->arr[q->b_index]=c; // na pozici aktualniho volneho indexu se ulozi hodnota c
        q->b_index=nextIndex(q->b_index); // a aktualni volny index se posune pomoci funkce nextIndex na dalsi pozici
    }
