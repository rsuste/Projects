...
**/
	...
	if ((*RootPtr) != NULL){
		//aktualni uzel
		tBTNodePtr akt_uzel = (*RootPtr);
		//otec aktualniho uzlu
		tBTNodePtr puv_uzel = NULL;
		while(akt_uzel != NULL){
			//hodnota je vetsi
			if(akt_uzel->Cont < Content){
				puv_uzel = akt_uzel;//ulozime si aktualni uzel
				akt_uzel = akt_uzel->RPtr;//a budeme pokracovat do praveho podstromu
			}
			//hodnota je mensi
			else if(akt_uzel->Cont > Content){
				puv_uzel = akt_uzel;
				akt_uzel = akt_uzel->LPtr;
		...
		//v pripade ze hodnota nebyla nalezena
		tBTNodePtr uzel = malloc(sizeof(struct tBTNode));//vytvorime novy uzel
		if (uzel != NULL){ // zda se alokace povedla
			uzel->Cont = Content; // vlozime hodnotu
			uzel->RPtr = NULL;
			uzel->LPtr = NULL;

			if(puv_uzel->Cont < Content){ // overime zda hodnota je vetsi nez hodnota otce
				puv_uzel->RPtr=uzel; //nove vytvoreny uzel ulozime jako pravy list otce
			}
			else puv_uzel->LPtr=uzel;//pokud je hodnota mensi tak jako levy list otce


		}
		else{
			//fprinf(stderr,"nepovedla se alokace noveho uzlu\n");
			return;
		}
	...
	//v pripade ze je strom prazdny
		...
		if (uzel != NULL) { // zda se alokace povedla
			...
			uzel->RPtr = NULL;
			uzel->LPtr = NULL;
			(*RootPtr) = uzel; // z uzlu udelame koren
		}
		else{
			//fprinf(stderr,"nepovedla se alokace noveho uzlu\n");
			return;
...
**/
	...
	//vytvorime si zasobnik hodnot
	tStackP zas_hod;
	//inicializujemeho
	SInitP(&(zas_hod));
	//pomocna hodnota
	tBTNodePtr pom= RootPtr;
	//naplnime zasobnik levymi uzly
	Leftmost_Preorder(pom,&(zas_hod));
	//dokud v zasobniku neco je pokracuj
	while(!(SEmptyP(&(zas_hod)))){
		//ulozis hodnotu prvku a odstranis jej ze zasobniku
		pom = STopPopP(&(zas_hod));
		//zpracujeme hodnoty praveho podstromu
		Leftmost_Preorder(pom->RPtr,&(zas_hod));
...
**/
	...
	while(!(SEmptyP(&(zas_hod)))){
		...
		BTWorkOut(pom);
		//zpracujeme hodnoty praveho podstromu
		Leftmost_Inorder(pom->RPtr,&(zas_hod));
...
**/
	...
	tStackP zas_hod;
	SInitP(&(zas_hod));
	//vytvorime si zasobnik bool
	tStackB zas_bool;
	SInitB(&(zas_bool));
	//pomocna hodnota
	tBTNodePtr pom= RootPtr;
	//naplnime zasobnik levymi uzly
	Leftmost_Postorder(pom,&(zas_hod),&(zas_bool));
	//dokud v zasobniku neco je pokracuj
	while(!(SEmptyP(&(zas_hod)))){
		//do pom si ulozime vrchol
		pom = STopPopP(&(zas_hod));
		//vratime
		SPushP(&(zas_hod),pom);
		int leva = STopPopB(&(zas_bool)); // prisli jsme k uzlu podruhe
		if (leva){
			SPushB(&(zas_bool),false);
			Leftmost_Postorder(pom->RPtr,&(zas_hod),&(zas_bool));//
		}
		else {
			//odstranim prvek ze zasobniku
			STopPopP(&(zas_hod));
			//zpracujeme uzel
			BTWorkOut(pom);
		}
...
**/
	...
	if ((*RootPtr)!= NULL){
		...
		do {
			if ((*RootPtr == NULL)){
				if (!SEmptyP(&(zas_hodnot))){ // pokud neni zasobnik prazdny
					(*RootPtr)=STopPopP(&(zas_hodnot));//vrchni polozka zasobniku se ulozi jako koren
				}
			}
			else{
				if ((*RootPtr)->RPtr != NULL){
					SPushP(&(zas_hodnot),(*RootPtr)->RPtr);
				}
				pom=(*RootPtr);
		...
		} while ((*RootPtr)!= NULL || (!SEmptyP(&(zas_hodnot))));
	}
